  ...
  return request(options)
    .then((response) => {
      ...
      if (!filteredNames.length) {
        return Promise.reject(new Error(`Digital.ai Web App Protection 6.1.0 could not be downloaded. Please contact ${SUPPORT_EMAIL} for help resolving this issue.`));
      }
      return { packageName: filteredNames[0].filename, accessToken };
    })
    .catch(() => Promise.reject(new Error('Digital.ai Web App Protection 6.1.0 could not be downloaded. Make sure you have the "Product download" entitlement associated with the API key.')));
};
...
const getPackage = (accessToken, filename) => {
  ...
  return download(options.uri, configuration.installLocation, options)
    .then(() => filename)
    .catch(error => Promise.reject(new Error(`Failed to download Digital.ai Web App Protection 6.1.0. Error message: ${error.message}`)));
};
...
const extractDmg = filename => new Promise((resolve, reject) => {
  ...
  dmg.mount(downloadedFile, (mountError, mountPath) => {
    if (mountError) reject(new Error(`Failed to mount Digital.ai Web App Protection 6.1.0 DMG package. ${mountError.message}`));
    fs.copySync(mountPath, configuration.installLocation);
    dmg.unmount(mountPath, (unmountError) => {
      if (unmountError) reject(new Error(`Failed to unmount Digital.ai Web App Protection 6.1.0 DMG package. ${unmountError.message}`));
