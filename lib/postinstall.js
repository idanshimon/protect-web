  ...
  return request(options)
    .then((response) => {
      ...
      if (!filteredNames.length) {
        exitWithMessage('Arxan for Web 3.2.0 could not be downloaded. Please contact support@arxan.com for help resolving this issue.');
      }
      return filteredNames[0].filename;
    })
    .catch(() => {
      exitWithMessage('Arxan for Web 3.2.0 is no longer available. Please consider upgrading to a newer version of Arxan for Web.');
    });
};
...
const getPackage = (accessToken, filename) => {
  ...
  return download(options.uri, configuration.installLocation, options)
    .catch((error) => {
      exitWithMessage(`Failed to download Arxan for Web 3.2.0. Error message: ${error.message}`);
    });
};
...
const extractDmg = (filename) => {
  ...
  dmg.mount(downloadedFile, (mountError, mountPath) => {
    if (mountError) exitWithMessage(`Failed to mount Arxan for Web 3.2.0 DMG package. ${mountError.message}`);
    fs.copySync(mountPath, configuration.installLocation);
    dmg.unmount(mountPath, (unmountError) => {
      if (unmountError) exitWithMessage(`Failed to unmount Arxan for Web 3.2.0 DMG package. ${unmountError.message}`);
