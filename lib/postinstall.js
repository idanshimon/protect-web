  ...
  return request(options)
    .then((response) => {
      ...
      if (!filteredNames.length) {
        exitWithMessage(`Digital.ai Web App Protection 4.8.0 could not be downloaded. Please contact ${SUPPORT_EMAIL} for help resolving this issue.`);
      }
      return filteredNames[0].filename;
    })
    .catch(() => {
      exitWithMessage('Digital.ai Web App Protection 4.8.0 could not be downloaded. Make sure you have the "Product download" entitlement associated with the API key.');
    });
};
...
const getPackage = (accessToken, filename) => {
  ...
  return download(options.uri, configuration.installLocation, options)
    .catch((error) => {
      exitWithMessage(`Failed to download Arxan for Web App 4.8.0. Error message: ${error.message}`);
    });
};
...
const extractDmg = (filename) => {
  ...
  dmg.mount(downloadedFile, (mountError, mountPath) => {
    if (mountError) exitWithMessage(`Failed to mount Arxan for Web App 4.8.0 DMG package. ${mountError.message}`);
    fs.copySync(mountPath, configuration.installLocation);
    dmg.unmount(mountPath, (unmountError) => {
      if (unmountError) exitWithMessage(`Failed to unmount Arxan for Web App 4.8.0 DMG package. ${unmountError.message}`);
